...
export default {
  ...
  methods: {
    login() {
      ...
      this.$http.post('/api/customer/login', credentials)
        .then(() => {
          this.loggingIn = false
          // 保存登录状态和凭证到本地存储
          localStorage.setItem('auth', 'customer')
          localStorage.setItem('customerUsername', this.form.username)
          // 保存Base64编码的凭证，用于后续请求的认证头
          const credentialsBase64 = btoa(`${this.form.username}:${this.form.password}`)
          localStorage.setItem('authToken', credentialsBase64)
          
          // 设置axios默认认证头
          this.$http.defaults.headers.common['Authorization'] = `Basic ${credentialsBase64}`
